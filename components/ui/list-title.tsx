...
const ListTitle = ({
  ...
  return (
    <div
      ...
      onClick={onClick}
      className={` ${className} ${onClick ? "touch-opacity" : ""} flex text-color py-[15px] px-[12px]  gap-[10px] w-full bg-[varr(--second-color)]  flex-row `}
    >
      <div
        style={leftStyle}
        className="flex flex-row items-center justify-center gap-[10px]"
      >
        <div style={leadingStyle} className=" leading flex items-center  ">
          {leading ?? <ChevronLeft />}
        </div>

        <div
          style={mainTextStyle}
          className="flex  gap-[2.5px] justify-start w-full flex-col "
        >
          ...
          {subTitle && (
            <div
              style={subtitleStyle}
              className="text-color whitespace-nowrap text-nowrap overflow-ellipsis w-full text-[11px] opacity-[0.75]"
            >
              {subTitle}
            </div>
          )}
